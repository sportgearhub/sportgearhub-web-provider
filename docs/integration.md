...
- `docs/api/provider-console-flows.design.md` — the console from phone sign-in to trading: picker, guided creation, checklist, review
- `docs/api/provider-onboarding-api.design.md` — the provider lifecycle API this console is built on
...
to a phone code. Never validate the passcode client-side — that hands back the attempt counter.
...
The console has one bootstrap call, `GET /api/v1/auth/me`, and lands the user by what it returns:

| `providers` | `pending_invitations` | Lands on |
|---|---|---|
| 0 | 0 | `/new` — guided creation (kind → ИНН → name + agreement) |
| 0 | >0 | the picker with the invitation block |
| 1 | 0 | straight into `/providers/{id}` |
| otherwise | | the picker; last-used cabinet preselected (remembered per device, never per account) |

- The URL carries the cabinet: `/providers/:providerId/…`. The layout copies it into the API client
  (`src/lib/active-provider.ts`), so `providerRequest` builds `/api/v1/providers/{id}/…` without
  every call site threading an id through.
- A `403` on a console route means the membership is gone: the layout sends the user back to the
  picker with «У вас нет доступа к этому кабинету».
- Kind and ИНН never change after creation; a different legal party is a different cabinet.
- Invitations are by phone. The invitee signs in with that number and finds the invitation on the
  picker; there are no links or emails.

See `docs/api/provider-console-flows.design.md` for the screens and `docs/api/provider-onboarding-api.design.md`
for the contracts.
